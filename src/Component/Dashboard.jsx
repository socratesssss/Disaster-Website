...
function Dashboard() {
  ...
  return (
    <section className='relative'>
      ...
      { isOpen ? <button onClick={toggler} className='fixed bottom-10 right-10 bg-orange-white w-16 h-16 p-3 rounded-full flex items-center justify-center shadow-2xl hover:bg-gray-400'>
        <CloseIcon className=''/>
      </button>
      :
      <button onClick={toggler} className='fixed bottom-10 right-10 bg-orange-500 w-16 h-16 p-3 rounded-full flex items-center justify-center shadow-lg hover:bg-orange-600'>
  ...
  {
    isOpen &&     <div className='w-[331px] h-[475px] bg-[#F4F4F5] absolute bottom-20 rounded-[12px] overflow-hidden right-14'>
